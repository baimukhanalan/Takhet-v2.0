...
const AdminAuthPage: React.FC<AdminAuthPageProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Updated credentials as requested: a@mail.ru / A
    setTimeout(() => {
      if (email === 'a@mail.ru' && password === 'A') {
      ...
      <div className="w-full max-w-lg space-y-10 relative z-10">
        ...
        <div className="bg-white/5 backdrop-blur-2xl p-10 md:p-14 rounded-[3.5rem] border border-white/10 shadow-2xl space-y-10">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button 
              disabled={isLoading}
              className={`w-full py-6 rounded-[2.5rem] font-black text-sm uppercase tracking-widest shadow-2xl flex items-center justify-center gap-4 transition-all active:scale-95 ${isLoading ? 'bg-slate-800 text-slate-600' : 'bg-primary text-white hover:bg-blue-600 shadow-primary/30'}`}
            >
        ...
        <p className="text-center text-slate-700 text-[10px] font-bold uppercase tracking-widest">
          Takhet+ Medical OS Security Protocol v4.2
        </p>
      </div>
    </div>
  );
