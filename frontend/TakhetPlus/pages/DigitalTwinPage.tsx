...
const DigitalTwin3DModel: React.FC<{ twin: DigitalTwinState }> = ({ twin }) => {
  ...
  return (
    <div 
      className="relative w-full h-[400px] md:h-[600px] flex items-center justify-center perspective-[2500px] select-none cursor-grab active:cursor-grabbing overflow-hidden rounded-[2.5rem] md:rounded-[4rem] bg-gradient-to-b from-slate-50/50 to-blue-50/30"
    ...
    <div className="max-w-7xl mx-auto pb-40 space-y-12 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-8 px-4 lg:px-0">
        <div className="flex items-center gap-6">
           ...
           <div>
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-900 tracking-tight uppercase">Digital <span className="text-primary italic">Twin</span></h1>
              <p className="text-slate-400 font-bold uppercase text-[8px] md:text-[9px] tracking-[0.4em] mt-1.5 flex items-center gap-2">
                <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse" /> Динамическая модель состояния
              </p>
           </div>
        </div>
        
        <div className="flex items-center gap-6 md:gap-10 bg-white p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] shadow-sm border border-slate-100 relative overflow-hidden group">
           ...
           <div className="flex items-center gap-3 md:gap-5 relative z-10">
              <div className="w-10 h-10 md:w-12 md:h-12 bg-blue-50 rounded-xl md:rounded-2xl flex items-center justify-center text-primary"><Gauge className="w-5 h-5 md:w-6 md:h-6" /></div>
              <div className="text-right">
                 <p className="text-[8px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest">Health Score</p>
                 <p className={`text-2xl md:text-3xl font-black transition-all ${isSimulating ? 'text-primary scale-110' : 'text-slate-900'}`}>
                    {isSimulating ? simulationResult?.score : twin.overallScore}%
                 </p>
              </div>
           </div>
           <div className="w-px h-8 md:h-10 bg-slate-100 relative z-10" />
           <div className="flex items-center gap-3 md:gap-5 relative z-10">
              <div className="w-10 h-10 md:w-12 md:h-12 bg-emerald-50 rounded-xl md:rounded-2xl flex items-center justify-center text-emerald-600"><Dna className="w-5 h-5 md:w-6 md:h-6" /></div>
              <div className="text-right">
                 <p className="text-[8px] md:text-[9px] font-bold text-slate-400 uppercase tracking-widest">Био-возраст</p>
                 <p className={`text-2xl md:text-3xl font-black transition-all ${isSimulating ? 'text-emerald-500 scale-110' : 'text-emerald-600'}`}>
