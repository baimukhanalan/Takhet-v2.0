...
import { 
  Stethoscope, ShieldCheck, TrendingUp, Zap, Clock, 
  Video, FileText, BrainCircuit, Users, Archive, 
  Award, BarChart3, GraduationCap, ChevronRight, Star, HeartPulse, Sparkles
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-white selection:bg-primary selection:text-white overflow-x-hidden">
          ...
          <div className="flex-1 space-y-10 text-center lg:text-left animate-in fade-in slide-in-from-left duration-1000 ease-out">
            <div className="space-y-6">
              ...
              <h2 className="text-xl md:text-4xl font-medium text-slate-800 leading-tight">
                Цифровая медицинская платформа для врачей, которые хотят зарабатывать проще, быстрее и без выгорания
              </h2>
            </div>
            
            <button 
              onClick={handleCTA} 
              className="group w-full sm:w-auto px-10 md:px-16 py-5 md:py-6 bg-primary text-white rounded-[2.5rem] font-black text-sm uppercase tracking-widest shadow-2xl shadow-primary/30 hover:shadow-primary/50 hover:scale-105 active:scale-95 transition-all duration-300 flex items-center justify-center gap-3 mx-auto lg:mx-0"
            >
              Зарегистрироваться
      ...
      <section className="py-32 px-6 md:px-20 bg-slate-50">
        <div className="max-w-5xl mx-auto space-y-16">
          ...
          <div className="bg-white p-8 md:p-20 rounded-[3rem] md:rounded-[5rem] shadow-xl border border-slate-100 space-y-12 hover:shadow-2xl transition-all duration-700">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 border-b border-slate-100 pb-12">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
               {[
                 { text: 'без абонплат', icon: ShieldCheck },
                 ...
                 <div key={i} className="text-center space-y-3 group cursor-default">
                    <div className="w-10 h-10 md:w-12 md:h-12 bg-slate-50 rounded-xl md:rounded-2xl flex items-center justify-center mx-auto text-primary group-hover:bg-primary group-hover:text-white transition-all duration-300"><item.icon className="w-5 h-5 md:w-6 md:h-6" /></div>
            ...
            <div className="pt-6 text-center px-4">
               <p className="text-lg md:text-xl font-bold text-slate-800 animate-pulse">👉 Вы начинаете зарабатывать с первого дня, без финансовых рисков.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-32 px-6 md:px-20 bg-white">
        <div className="max-w-7xl mx-auto space-y-24">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="bg-slate-50 p-8 md:p-12 rounded-[3rem] md:rounded-[4rem] border border-slate-100 space-y-8 hover:shadow-xl hover:-translate-y-2 transition-all duration-500 group">
            ...
            <div className="bg-slate-900 p-8 md:p-12 rounded-[3rem] md:rounded-[4rem] text-white space-y-8 shadow-2xl hover:-translate-y-2 transition-all duration-500 group relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              ...
              { title: 'Digital Twin пациента', desc: 'Вы видите текущее состояние пациента, динамику, потенциальные риски. Это контекст, а не разрозненные файлы.', icon: BrainCircuit },
              ...
              { title: 'Безопасность и легальность', desc: 'Официальный договор (ИП / клиника), хранение данных в РК, прозрачная история решений. Защита врача.', icon: ShieldCheck },
            ].map((feature, i) => (
              <div key={i} className="bg-white p-10 rounded-[3rem] md:rounded-[3.5rem] border border-slate-100 hover:shadow-2xl hover:-translate-y-2 transition-all group flex flex-col justify-between">
                <div>
                   <div className="w-16 h-16 bg-primary/5 text-primary rounded-[1.5rem] flex items-center justify-center mb-8 group-hover:bg-primary group-hover:text-white transition-all duration-500">
                     <feature.icon className="w-8 h-8 group-hover:rotate-12 transition-transform" />
                   </div>
            ...
            <div className="bg-primary p-10 rounded-[3rem] md:rounded-[3.5rem] text-white flex flex-col justify-between hover:scale-[1.02] transition-transform shadow-2xl relative overflow-hidden group">
               ...
               <button onClick={handleCTA} className="mt-8 py-5 bg-white text-primary rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl relative z-10 hover:bg-slate-100 transition-colors">Подробнее</button>
        ...
        <div className="max-w-7xl mx-auto space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            {[
              ...
              <div key={i} className="bg-slate-50 p-8 md:p-12 rounded-[3rem] md:rounded-[4rem] border border-slate-100 space-y-8 flex flex-col justify-between shadow-sm hover:shadow-xl hover:scale-[1.01] transition-all duration-500">
        ...
        <div className="max-w-7xl mx-auto space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative">
            {[
              ...
              <div key={i} className="relative p-10 md:p-12 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[3rem] md:rounded-[3.5rem] text-center group hover:bg-white/10 transition-all duration-500">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-primary text-white rounded-full flex items-center justify-center font-black text-2xl md:text-3xl mx-auto mb-8 border-4 border-white/10 shadow-2xl transition-transform group-hover:scale-110 group-hover:rotate-12">{s.id}</div>
          ...
          <div className="text-center pt-10 px-4">
             <button onClick={handleCTA} className="group w-full sm:w-auto px-8 md:px-16 py-6 md:py-8 bg-white text-slate-950 rounded-[2rem] md:rounded-[2.5rem] font-black text-base md:text-xl uppercase tracking-widest shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3 mx-auto">
      ...
      <section className="py-40 px-6 bg-white text-center relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto space-y-12 relative z-10 px-4">
          ...
          <div className="pt-10">
            <button onClick={handleCTA} className="w-full sm:w-auto px-10 md:px-16 py-6 md:py-8 bg-primary text-white rounded-[2rem] md:rounded-[2.5rem] font-black text-base md:text-xl uppercase tracking-widest shadow-2xl hover:scale-110 hover:shadow-primary/40 active:scale-95 transition-all duration-300">Стать частью системы</button>
          </div>
        </div>
      </section>
