...
import { Calendar, Clock, ChevronRight, CheckCircle2, MoreVertical, Play, Download, Plus, X } from 'lucide-react';
...
const PatientAppointments: React.FC = () => {
  ...
  const [quickCaseSummary, setQuickCaseSummary] = useState('');
  ...
  const createQuickCase = async () => {
    if (!quickCaseSummary.trim()) return;
    try {
      const createdCase = await roleApi.patientCreateCase(quickCaseSummary.trim());
      await roleApi.createPaymentIntent(createdCase?.id, 15000);
      setStatusText('Кейс создан и платеж инициализирован.');
      ...
      setAppointments(MockDB.getAppointments());
      setStatusText('Кейс сохранён локально (offline fallback).');
      setIsQuickCaseOpen(false);
      setQuickCaseSummary('');
    }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-16 pb-32">
      {isQuickCaseOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] p-8 space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-2xl font-black">Быстрый кейс</h3>
              <button onClick={() => setIsQuickCaseOpen(false)} className="p-3 rounded-xl bg-slate-100"><X className="w-5 h-5" /></button>
            </div>
            <textarea
              value={quickCaseSummary}
              onChange={(e) => setQuickCaseSummary(e.target.value)}
              className="w-full h-40 bg-slate-50 rounded-2xl p-4 font-medium"
              placeholder="Кратко опишите симптомы..."
            />
            <button onClick={createQuickCase} className="w-full py-4 bg-primary text-white rounded-2xl font-black uppercase text-xs tracking-widest">
              Создать кейс
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <button onClick={() => setIsQuickCaseOpen(true)} className="px-10 py-5 bg-primary text-white rounded-[2rem] font-black text-sm uppercase tracking-widest hover:bg-blue-800 transition-all shadow-sm">
            Быстрый кейс
          </button>
        </div>
      </div>
