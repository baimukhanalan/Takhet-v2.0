...
const AdminDashboard: React.FC<{ user: User; onLogout: () => void }> = ({ user, onLogout }) => {
  ...
  const handleSendAi = async () => {
    ...
    try {
      const response = await api<{ aiResult?: { raw?: string } }>('/triage', {
        method: 'POST',
        body: JSON.stringify({ symptoms: `Admin assistant request: ${userText}` })
      });

      const aiMsg: AIChatMessage = { 
        role: 'model', 
        text: response.aiResult?.raw || "Ошибка связи с ядром ИИ.", 
  ...
  const handleRequestAction = async (id: string, status: 'Approved' | 'Rejected') => {
    try {
      ...
      setAdminActionStatus(`Request ${id} -> ${status} (backend sync ok)`);
    } catch {
      MockDB.updateRequestStatus(id, status);
      setAdminActionStatus(`Request ${id} -> ${status} (local fallback)`);
    }
  };
  ...
  const handleDeleteDoctor = async (id: string) => {
    try {
      await roleApi.adminDeleteUser(id);
      setAdminActionStatus(`Doctor ${id} deleted via backend`);
    } catch {
      setAdminActionStatus(`Doctor ${id} delete fallback`);
    }
  ...
  const handleDeleteComplaint = async (id: string) => {
    try {
      await roleApi.adminBroadcast('Complaint removed', `Complaint ${id} was removed by admin`);
      setAdminActionStatus(`Complaint ${id} removed + broadcast sent`);
    } catch {
      setAdminActionStatus(`Complaint ${id} removed locally`);
    }
    MockDB.deleteComplaint(id);
  };
  ...
  const handleDeleteReview = async (id: string) => {
    try {
      ...
      if (firstDoctor) {
        await roleApi.adminAssignCase(id, firstDoctor.id);
      }
      setAdminActionStatus(`Review ${id} action synced`);
    } catch {
      setAdminActionStatus(`Review ${id} removed locally`);
    }
  ...
  const handleBroadcastMaintenance = async () => {
    try {
      await roleApi.adminBroadcast('System Notice', 'Planned maintenance update from admin panel');
      setAdminActionStatus('Broadcast sent successfully');
    } catch {
      setAdminActionStatus('Broadcast failed (backend unavailable)');
    }
  };
      ...
      case 'overview':
        return (
          <div className="space-y-6 lg:space-y-10 animate-in fade-in duration-500">
            {adminActionStatus && <div className="px-4 py-3 bg-primary/5 text-primary rounded-2xl text-[10px] font-black uppercase tracking-widest">{adminActionStatus}</div>}
             ...
             <div className={`${styles.card} lg:rounded-[3.5rem] flex flex-col overflow-hidden border-x-0 lg:border-x min-h-[600px] lg:min-h-[900px] shadow-2xl relative`}>
                ...
                <div className={`p-4 lg:p-12 border-t shrink-0 ${isDark ? 'bg-slate-900 border-white/5' : 'bg-white border-slate-100'}`}>
                   <div className={`flex items-center gap-2 lg:gap-6 ${styles.input} border ${styles.border} rounded-[2.5rem] lg:rounded-[6rem] p-1.5 lg:p-4 pl-8 lg:pl-16 focus-within:ring-[12px] focus-within:ring-primary/5 transition-all shadow-inner`}>
                      ...
                      <button onClick={handleBroadcastMaintenance} className="w-full py-4 bg-primary text-white rounded-2xl font-black text-[10px] uppercase tracking-widest">Broadcast Notice</button>
                   </div>
                </div>
             </div>
                    ...
                    <tbody className={`divide-y ${styles.border}`}>
                        {reviews.map(rev => (
                          <tr key={rev.id} className="hover:bg-primary/5 transition-colors">
                            ...
                            <td className="p-6">
                                <button onClick={() => handleDeleteReview(rev.id)} className="text-red-400 hover:text-red-600 p-2"><Trash2 className="w-4 h-4"/></button>
                            </td>
                          </tr>
                        ))}
                    </tbody>
                ...
                {complaints.map(comp => (
                  <div key={comp.id} className={`${styles.card} p-6 lg:p-8 rounded-[1.5rem] lg:rounded-[2.5rem] border flex flex-col md:flex-row items-center justify-between gap-4 hover:border-primary/20 transition-all`}>
                     ...
                     <div className="flex items-center gap-4 lg:gap-6 shrink-0 w-full md:w-auto justify-between md:justify-start">
                        ...
                        <button onClick={() => handleDeleteComplaint(comp.id)} className="p-3 lg:p-4 bg-red-500/10 text-red-500 rounded-xl lg:rounded-2xl hover:bg-red-500 hover:text-white transition-all">
                        ...
                        {doctors.map(doc => (
                          <tr key={doc.id} className="hover:bg-primary/5 transition-colors">
                            ...
                            <td className="p-6 flex gap-2">
                                ...
                                <button onClick={() => handleDeleteDoctor(doc.id)} className={`${styles.textSub} hover:text-red-500 transition-colors`}><Trash2 className="w-5 h-5" /></button>
                            </td>
                          </tr>
                        ))}
                    </tbody>
