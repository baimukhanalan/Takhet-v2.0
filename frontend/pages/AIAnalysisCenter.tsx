...
const AIAnalysisCenter: React.FC = () => {
  ...
  const handleAnalysis = async () => {
    ...
    try {
      const triage = await api<{ aiResult?: { raw?: string; riskLevel?: string } }>("/triage", {
        method: 'POST',
        body: JSON.stringify({ symptoms: contentStr })
      });

      const data = {
        summary: triage.aiResult?.raw || 'AI response unavailable',
        ...
        nextSteps: 'Создайте консультационный кейс и оплатите консультацию через Kaspi Pay.'
      };
