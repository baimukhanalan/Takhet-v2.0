...
import { 
  ...
  Fingerprint, Archive, Users, Stethoscope, BarChart3, Menu, X, Heart, Search
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ role, onLogout }) => {
  ...
  const patientLinks = [
    ...
    { to: '/health-browser', icon: Search, label: 'AI Браузер' },
    { to: '/takhet-ai', icon: MessageSquare, label: 'Takhet AI' },
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="p-6 border-t border-border">
        <button
          ...
          className="flex items-center gap-4 w-full px-5 py-4 text-muted hover:text-red-600 hover:bg-red-50 rounded-2xl transition-all font-bold uppercase tracking-widest text-[11px]"
        >
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-3 left-4 z-[60]">
        <button onClick={() => setIsMobileMenuOpen(true)} className="p-3 bg-background border border-border rounded-xl shadow-lg text-primary">
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[100] lg:hidden">
          ...
          <div className="absolute top-0 left-0 w-80 h-full bg-background animate-in slide-in-from-left duration-300 shadow-2xl">
            <button onClick={() => setIsMobileMenuOpen(false)} className="absolute top-8 right-6 p-2 text-muted-foreground"><X className="w-6 h-6" /></button>
            <NavContent />
          </div>
        </div>
      )}
