...
export const DoctorRevenueChart: React.FC<{ data: RevenuePoint[] }> = ({ data }) => (
  <ResponsiveContainer width="100%" height="100%">
    <BarChart data={data}>
      ...
      <Bar dataKey="amount" fill="#1D4ED8" radius={[8, 8, 8, 8]} barSize={20}>
        {data.map((entry, i) => <Cell key={entry.name} fill={i === data.length - 1 ? '#1D4ED8' : '#cbd5e1'} />)}
      </Bar>
    </BarChart>
  </ResponsiveContainer>
);
...
export const PartnerPerformanceChart: React.FC<{ data: PartnerPoint[] }> = ({ data }) => (
  <ResponsiveContainer width="100%" height="100%">
    <BarChart data={data}>
      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E7EBF4" />
      <XAxis dataKey="month" axisLine={false} tickLine={false} />
      <YAxis hide />
      <Tooltip cursor={{ fill: '#F1F3F9' }} contentStyle={{ borderRadius: '1.5rem', border: 'none' }} />
      <Bar dataKey="revenue" radius={[10, 10, 0, 0]} fill="#1D4ED8" />
    </BarChart>
  </ResponsiveContainer>
);
...
export const AdminSystemLoadChart: React.FC<{ data: HealthPoint[]; isDark: boolean }> = ({ data, isDark }) => (
  <ResponsiveContainer width="100%" height="100%">
    <AreaChart data={data}>
      ...
      <Area type="monotone" dataKey="cpu" stroke="#1D4ED8" fillOpacity={1} fill="url(#colorCpu)" strokeWidth={4} />
    </AreaChart>
  </ResponsiveContainer>
);
...
export const AdminRevenueChart: React.FC<{ data: AnalyticsPoint[]; isDark: boolean }> = ({ data, isDark }) => (
  <ResponsiveContainer width="100%" height="100%">
    <BarChart data={data}>
      ...
      <Bar dataKey="amount" fill="#1D4ED8" radius={[10, 10, 10, 10]} barSize={25} />
    </BarChart>
  </ResponsiveContainer>
);
...
export const AdminAudienceChart: React.FC<{ data: AnalyticsPoint[]; isDark: boolean }> = ({ data, isDark }) => (
  <ResponsiveContainer width="100%" height="100%">
    <LineChart data={data}>
      ...
      <YAxis hide />
      <Tooltip contentStyle={{ backgroundColor: isDark ? '#0f172a' : '#fff', borderRadius: '1rem', border: 'none' }} />
      <Line type="monotone" dataKey="users" stroke="#7C8EE0" strokeWidth={4} dot={{ fill: '#7C8EE0', r: 6 }} />
    </LineChart>
  </ResponsiveContainer>
);
