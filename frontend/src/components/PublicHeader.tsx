...
import { Globe, Menu, X, Check, Moon, Sun } from 'lucide-react';
...
const PublicHeader: React.FC<PublicHeaderProps> = ({ activePath }) => {
  ...
  const navLinks = [
    ...
    { name: t.nav.takhetAI, path: '/takhet-ai' },
  ...
  return (
    <>
      <header className={`fixed top-0 w-full z-[200] transition-all duration-500 h-20 flex items-center justify-between px-6 md:px-20 ${isMenuOpen ? 'bg-transparent border-transparent' : 'bg-background/70 backdrop-blur-2xl border-b border-border'}`}>
            ...
            {navLinks.map((link) => (
              <Link 
                ...
                className={`text-[11px] font-black transition-all uppercase tracking-[0.2em] ${
                  activePath === link.path ? 'text-primary' : 'text-muted-foreground hover:text-primary'
                }`}
              >
                {link.name}
              </Link>
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            <button 
              ...
              className="p-3 rounded-2xl bg-secondary text-muted-foreground hover:bg-secondary/80 transition-all flex items-center gap-2"
            >
          ...
          <div className="hidden md:flex items-center gap-3">
            <button 
              onClick={() => navigate('/auth', { state: { mode: 'login', from: location } })} 
              className="text-xs font-black uppercase tracking-widest text-muted-foreground px-6 py-3 hover:bg-secondary rounded-2xl transition-all"
            >
              {t.nav.auth}
            </button>
            <button 
              onClick={() => navigate('/auth', { state: { mode: 'register', from: location } })} 
          ...
          <button 
            ...
            className={`p-3 rounded-2xl transition-all active:scale-90 ${isMenuOpen ? 'bg-background text-primary rotate-90' : 'bg-secondary text-foreground md:hidden'}`}
          >
        ...
        <div className="fixed inset-0 z-[190] bg-primary flex flex-col pt-32 px-10 animate-in fade-in zoom-in-95 duration-500">
          ...
          <div className="mt-auto pb-16 space-y-6 relative z-10">
            <div className="flex gap-4">
              <button 
                onClick={() => { setIsMenuOpen(false); navigate('/auth', { state: { mode: 'login', from: location } }); }} 
              ...
              <button 
                onClick={() => { setIsMenuOpen(false); navigate('/auth', { state: { mode: 'register', from: location } }); }} 
