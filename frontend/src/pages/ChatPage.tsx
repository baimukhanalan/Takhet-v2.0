...
import { Search, Send, Image as ImageIcon, Paperclip, ChevronRight, CheckCircle2, Lock, UserCircle2, Clock, MessageSquare } from 'lucide-react';
...
const ChatPage: React.FC<{ user: User }> = ({ user }) => {
  ...
  useEffect(() => {
    const load = () => setChats(MockDB.getChats());
    load();
  ...
  const handleFinish = () => {
    ...
    if (confirm('Завершить консультацию? Это запустит 5-дневный таймер до блокировки чата.')) {
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start mb-1">
                  <h4 className="font-bold break-words text-foreground">{user.role === UserRole.DOCTOR ? chat.patientName : chat.doctorName}</h4>
                  <span className="text-[10px] text-muted-foreground font-medium uppercase">{chat.lastTimestamp}</span>
                </div>
                <p className="text-xs text-muted-foreground break-words font-medium">{chat.lastMessage}</p>
              </div>
            </div>
          ))}
        </div>
