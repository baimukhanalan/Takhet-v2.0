        ...
        <div className={isPortal ? "space-y-8 md:space-y-12" : "lg:col-span-8 space-y-8 md:space-y-12"}>
          <FadeIn direction="up">
            <div className="space-y-3 md:space-y-4">
              <h1 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tighter uppercase leading-tight">{t.community.title}</h1>
              <p className="text-slate-400 font-bold uppercase text-[9px] md:text-[10px] tracking-[0.3em] md:tracking-[0.4em]">{t.community.desc}</p>
            </div>
          </FadeIn>
            ...
            <FadeIn direction="up" delay={0.1}>
              <div className="bg-white p-6 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-100 shadow-sm space-y-6 md:space-y-8">
                <div className="flex items-center gap-4 md:gap-5">
                   <div className="w-10 h-10 md:w-12 md:h-12 bg-primary/5 rounded-xl md:rounded-2xl flex items-center justify-center text-primary"><MessageSquare className="w-5 h-5 md:w-6 md:h-6" /></div>
                   <h3 className="font-black text-lg md:text-xl uppercase tracking-tight">{t.community.postTitle}</h3>
                </div>
                <div className="space-y-4">
                  <input value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder={t.community.postTopic} className="w-full bg-slate-50 border-none rounded-xl md:rounded-2xl px-5 md:px-6 py-4 md:py-5 font-bold outline-none focus:ring-2 ring-primary/10 transition-all text-sm md:text-base" />
                  <textarea value={newBody} onChange={e => setNewBody(e.target.value)} placeholder={t.community.postBody} className="w-full bg-slate-50 border-none rounded-2xl md:rounded-3xl px-5 md:px-6 py-4 md:py-5 min-h-[120px] md:min-h-[140px] text-xs md:text-sm font-medium outline-none focus:ring-2 ring-primary/10 transition-all" />
                </div>
            ...
            <div className="flex items-center gap-1.5 bg-slate-50 p-1 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 w-full sm:w-fit overflow-x-auto no-scrollbar">
              {(['All', 'Unanswered'] as const).map(f => (
                <button 
                  ...
                  className={`flex-1 sm:flex-none px-6 md:px-8 py-2.5 md:py-3 rounded-[1.2rem] md:rounded-[1.5rem] text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all relative whitespace-nowrap ${activeFilter === f ? 'text-primary' : 'text-slate-400 hover:text-slate-600'}`}
                >
                  {activeFilter === f && (
                    <motion.div 
                  ...
                  <span className="relative z-10">{f === 'All' ? t.community.filterAll : t.community.filterUnanswered}</span>
                </button>
              ))}
            </div>
          </FadeIn>
                ...
                <div className="bg-white p-6 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-100 shadow-sm hover:border-primary/20 transition-all space-y-6 md:space-y-8 group">
                  <div className="flex justify-between items-start">
                    <div className="flex flex-wrap items-center gap-3 md:gap-4">
                      <span className="px-3 md:px-4 py-1.5 md:py-2 bg-slate-50 text-slate-500 text-[8px] md:text-[9px] font-black rounded-lg md:rounded-xl uppercase tracking-widest border border-slate-100">{c.category}</span>
                      <span className="text-[8px] md:text-[9px] text-slate-300 font-black uppercase tracking-[0.1em] md:tracking-[0.2em] flex items-center gap-1.5 md:gap-2"><UserCircle2 className="w-3.5 h-3.5 md:w-4 md:h-4" /> {c.author}</span>
                    </div>
                    {c.replies.length > 0 && <CheckCircle className="w-5 h-5 md:w-6 md:h-6 text-emerald-500 shrink-0" />}
                  </div>
                  
                  <div>
                    <h4 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight leading-tight">{c.title}</h4>
                    <p className="text-slate-500 text-sm md:text-base leading-relaxed mt-3 md:mt-4 font-medium">{c.body}</p>
                  </div>
                  ...
                  <AnimatePresence>
                    {c.replies.length > 0 && (
                      ...
                      >
                        {c.replies.map((r, i) => (
                          <div key={i} className="bg-blue-50/30 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border border-blue-100/50 space-y-2 md:space-y-3 relative">
                            <div className="flex items-center gap-2 md:gap-3 text-[9px] md:text-[10px] font-black text-primary uppercase tracking-widest">
                              <Award className="w-4 h-4 md:w-5 md:h-5 text-indigo-500" /> {r.author} <span className="text-slate-400 font-bold lowercase"> • {t.community.doctorReply}</span>
                            </div>
                            <p className="text-sm md:text-base text-slate-800 font-bold italic leading-relaxed">"{r.text}"</p>
                          </div>
                        ))}
                      </motion.div>
                    )}
                  ...
                  {isPortal && isDoctor && (
                    <div className="pt-4 md:pt-6 space-y-4 md:space-y-6 border-t border-slate-50">
                      ...
                      <div className="flex items-center justify-center gap-2 md:gap-3">
                         ...
                         <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.community.reputationPoints} [+50 pts]</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-6 md:gap-8 pt-2 md:pt-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-300">
                    ...
                    >
                      <ThumbsUp className="w-3.5 h-3.5 md:w-4 md:h-4 group-hover/btn:scale-125 transition-transform" /> {c.likes}
                    </motion.button>
                    <span className="flex items-center gap-2"><MessageSquare className="w-3.5 h-3.5 md:w-4 md:h-4" /> {c.replies.length}</span>
                  </div>
                </div>
              </FadeIn>
            )) : (
              <div className="py-20 md:py-32 text-center opacity-20 font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-lg md:text-xl">Нет вопросов в этой категории</div>
            )}
          </FadeInStagger>
        </div>
                    ...
                    {topDoctors.map((doc, i) => (
                      <FadeIn key={doc.id}>
                        <div className="flex items-center justify-between group">
                          <div className="flex items-center gap-4 md:gap-5">
                             <div className="relative">
                                <img src={doc.avatar} className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl object-cover border-2 border-white shadow-sm" alt="" referrerPolicy="no-referrer" />
                                <span className="absolute -top-1.5 -left-1.5 w-6 h-6 md:w-7 md:h-7 bg-slate-900 text-white rounded-full flex items-center justify-center text-[9px] md:text-[10px] font-black border-2 border-white">{i+1}</span>
                             </div>
                             <div className="min-w-0">
                                <p className="font-black text-sm md:text-base text-slate-800 leading-tight group-hover:text-primary transition-colors break-words">{doc.name}</p>
                                <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase mt-1 tracking-widest">{doc.specialty}</p>
                             </div>
                          </div>
                          <div className="text-right">
                             <p className="text-xs md:text-sm font-black text-indigo-600">{doc.reputationPoints}</p>
