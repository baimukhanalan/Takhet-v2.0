...
import { BrainCircuit, ChevronRight, Maximize2, Search, Send, ShieldAlert, Sparkles, Star, X } from 'lucide-react';
...
const SoulAssistant: React.FC<{ onOpenFull: () => void; publicMode: boolean; guestMode: boolean }> = ({ onOpenFull, publicMode, guestMode }) => {
  ...
  const shell = (
    <div className="w-full h-full bg-white rounded-[4rem] shadow-[0_0_100px_rgba(15,23,42,0.16)] flex flex-col overflow-hidden border border-white/20">
      <div className="p-8 md:p-10 bg-slate-950 text-white flex items-center justify-between shrink-0 relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(100,181,246,0.24),transparent_34%)]" />
        <div className="flex items-center gap-6 relative z-10">
          ...
          <div>
            <h3 className="text-xl md:text-2xl font-black uppercase tracking-tighter leading-none">Takhet AI</h3>
            <p className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.3em] opacity-80 mt-1.5">{publicMode ? 'Душевный режим' : 'Поддержка рядом'}</p>
          </div>
        </div>
        <button onClick={onOpenFull} className="p-4 md:p-5 bg-white/10 hover:bg-white/20 border-2 border-white/40 rounded-3xl transition-all flex items-center justify-center shadow-xl relative z-10" aria-label="Развернуть блок">
          <Maximize2 className="w-6 h-6" />
        </button>
      </div>

      <div className="flex-1 overflow-y-auto p-8 md:p-12 space-y-8 bg-[linear-gradient(180deg,rgba(239,246,255,0.72),rgba(255,255,255,0.95))] no-scrollbar">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center space-y-8 py-16">
            <div className="w-28 h-28 bg-blue-50 rounded-full flex items-center justify-center relative">
      ...
      <div className="p-8 md:p-10 border-t border-slate-100 bg-white shrink-0 space-y-4">
        <div className="flex items-center gap-4 bg-slate-50 border-2 border-slate-50 rounded-[2rem] md:rounded-[4rem] p-3 pl-8 md:pl-10 transition-all focus-within:ring-8 focus-within:ring-blue-500/5 focus-within:bg-white focus-within:border-blue-100">
          ...
          <button onClick={() => void handleSend()} disabled={!input.trim() || isTyping} className="w-16 h-16 md:w-20 md:h-20 bg-slate-950 text-white rounded-full flex items-center justify-center shadow-2xl hover:bg-blue-600 transition-all disabled:opacity-30">
          ...
          <>
            <section className="grid grid-cols-1 lg:grid-cols-12 gap-10 sm:gap-12 lg:gap-16 items-center">
              <div className="lg:col-span-7 space-y-12">
                ...
                <div className="inline-flex items-center gap-4 px-8 py-3 bg-blue-100 text-blue-700 rounded-full text-[11px] font-black uppercase tracking-[0.3em] border border-blue-200 shadow-sm"><Sparkles className="w-5 h-5" /> Mental</div>
                  <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-slate-950 leading-[0.9] tracking-tighter uppercase break-words">
                    Takhet AI <br />
                    <span className="text-blue-500 italic lowercase font-normal break-words">Mental</span>
                  </h1>
                  ...
                  <button onClick={openSoulfulMode} className="px-8 sm:px-12 md:px-16 py-5 sm:py-6 md:py-8 bg-slate-950 text-white rounded-[3rem] font-black text-[12px] uppercase tracking-[0.3em] shadow-[0_30px_60px_rgba(0,0,0,0.2)] hover:bg-blue-600 transition-all">Начать сейчас</button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
                  <div className="p-10 bg-white rounded-[4rem] border border-slate-100 shadow-xl space-y-6">
                    <div className="w-16 h-16 bg-blue-50 rounded-2xl flex items-center justify-center"><ShieldAlert className="w-8 h-8 text-blue-500" /></div>
            ...
            <section className="space-y-8">
              <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
                <div className="space-y-4 max-w-3xl">
                  <p className="text-[10px] uppercase tracking-[0.35em] font-black text-primary">Специалисты</p>
                  <h2 className="text-3xl sm:text-5xl font-black tracking-tight text-slate-950 leading-none">Запись к психологу или психотерапевту</h2>
                  <p className="text-slate-500 font-medium text-lg leading-relaxed">Откройте профиль специалиста, выберите доступную дату и время. Для гостевой записи консультация продолжится через одноразовый сценарий без сохранения саммари в медархив.</p>
                </div>
                <div className="bg-white p-4 md:p-5 rounded-[2.5rem] border border-slate-100 shadow-sm grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-3 w-full lg:max-w-2xl">
              ...
              {filteredSpecialists.length === 0 ? (
                <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm text-center">
                  <p className="text-slate-500 font-bold">По выбранным фильтрам специалисты не найдены. Сбросьте поиск или выберите другое направление.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {filteredSpecialists.map((doctor) => (
                    <div key={doctor.id} data-takhet-tilt className="bg-white p-6 md:p-8 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group flex flex-col">
                      ...
                      <button data-takhet-magnetic-button onClick={() => openDoctor(doctor)} className="w-full py-5 bg-slate-900 text-white rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest mt-6 group-hover:bg-primary shadow-xl transition-all flex items-center justify-center gap-2">
                        Открыть профиль и время <ChevronRight className="w-4 h-4" />
                      </button>
                    </div>
                  ))}
                </div>
