...
assert(!landing.includes('customCursorRef'), 'Landing must use the shared global custom cursor instead of a duplicate local cursor');
assert(!landing.includes('takhet-custom-cursor'), 'Landing must not render a second custom cursor');
...
assert(css.includes('.takhet-platform-cursor'), 'CSS must define the shared custom plus cursor');
assert(css.includes('cursor: none'), 'The shared motion shell must hide the default cursor where the custom cursor is active');
assert(css.includes('.takhet-platform-motion-shell *'), 'The shared cursor must cover all nested hover targets');
