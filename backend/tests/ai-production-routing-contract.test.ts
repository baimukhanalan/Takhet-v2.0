...
const database = read('src/database/database.providers.ts');
const env = read('src/config/env.config.ts');
...
assert(env.includes('databasePoolMax'), 'production database pool size must be configurable');
assert(database.includes('max: env.databasePoolMax'), 'each backend instance must use a bounded database pool');
