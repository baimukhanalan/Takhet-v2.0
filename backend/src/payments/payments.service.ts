...
export class PaymentsService {
  ...
  async reversePayout(payoutId: string, actorId: string, reason: string) {
    ...
    const payoutDate = payout.paid_at || payout.created_at;
    const ageDays = Math.floor((Date.now() - new Date(payoutDate).getTime()) / (24 * 60 * 60 * 1000));
    if (ageDays > env.payoutReversalWindowDays) {
      return {
        reversed: false,
        reason: 'window_expired',
        ageDays,
        maxWindowDays: env.payoutReversalWindowDays
      };
    }
    ...
    await this.auditService.log('payout.reversed', actorId, { payoutId, reason, ageDays });
    return { reversed: true, ageDays };
  }
