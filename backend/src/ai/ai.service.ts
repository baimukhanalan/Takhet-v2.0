...
export class AiService {
  ...
  async getHealthInsights(query: string) {
    if (this.isTakhetNavigationQuery(query)) {
      return this.buildTakhetNavigationInsight(query);
    }
  ...
  private isTakhetNavigationQuery(query: string) {
    return /(запис|запись|консультац|консульт|каталог|найти врача|подобрать врача|онлайн[-\s]?при[её]м|стоимость приема|цена приема|слот|расписан)/i.test(
      query
    );
  }

  private buildTakhetNavigationInsight(query: string) {
    return {
      query,
      summary: {
        likelyCause:
